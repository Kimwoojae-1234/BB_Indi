...
{
    public class PitchOriginPv : MonoBehaviour
    {

        public const float STRIKE_ZONE_WIDTH = 1 * 14.0f;// (2.73f * 4.5f);//
        public const float STRIKE_ZONE_HEIGHT = 1.228f * 14.0f;// (3.1f * 4.5f);//
        ...
        /*
        public void setPitcherTest(int posX, int posY)
        {
            bMyTurn = false;

            INIT_BALL_GAB = posX;
            INIT_BALL_HEIGHT = posY;
        ...
        private float ballTypeSpeedRate = 1.0f;
        private void initBallPosition(Pitcher pitcher)
        {
            INIT_BALL_GAB = 0;
            int height = 12;

            if (Mode.bPvpMode == true) ballTypeSpeedRate = 1.0f;

            if (ballType == PitchingArsenal.FASTBALL)
            {
                //직구  스피드레이트 1.8f : 시작높이 12
                if (Mode.bPvpMode == false) ballTypeSpeedRate = 1.8f;
                height = 12;
            }
            else if (ballType == PitchingArsenal.UPSHOOT)
            {
                //업슛 :  최대 무브먼트 5,  스피드레이트 1.2f : 시작높이 -2 : //프로그램적으로 스트레이트 계열
                if (Mode.bPvpMode == false) ballTypeSpeedRate = 1.2f;
                height = -2;
            }
            else if (ballType == PitchingArsenal.FORK || ballType == PitchingArsenal.H_FORK)
            {
                //포크볼 : 최대 무브먼트 5,  스피드레이트 1.5f  시작높이 7    //프로그램적으로 오프스피드 계열
                if (Mode.bPvpMode == false) ballTypeSpeedRate = 1.5f;
                height = 7;
            }
            else if (ballType == PitchingArsenal.SFF)
            {
                ...
                //SFF
                if (Mode.bPvpMode == false) ballTypeSpeedRate = 1.7f;
                height = 8;
            }
            else if (ballType == PitchingArsenal.TWOSEAM || ballType == PitchingArsenal.SINKING_FAST || ballType == PitchingArsenal.CUT_FAST)
            {
            ...
            else if (ballType == PitchingArsenal.SLIDER || ballType == PitchingArsenal.H_SLIDER || ballType == PitchingArsenal.SLURVE
                   || ballType == PitchingArsenal.SINKER || ballType == PitchingArsenal.FRISBEE)
            {
            ...
            else if (ballType == PitchingArsenal.CURVE || ballType == PitchingArsenal.POWER_CURVE || ballType == PitchingArsenal.SLOW_CURVE
                   || ballType == PitchingArsenal.POKPOSU_CURVE || ballType == PitchingArsenal.KNUCKLE_CURVE)
            {
                //커브류 : 최대 무브먼트 5,  스피드레이트 1.2f : 시작높이 8
                //커브류
                if (Mode.bPvpMode == false) ballTypeSpeedRate = 1.3f;
                height = 8;
            }
            else
            {
                ...
                //기타 오프스피드 계열
                if (Mode.bPvpMode == false) ballTypeSpeedRate = 1.4f;
                height = 13;
            }
            ...
            {
                //언더핸드
                if (ballType == PitchingArsenal.UPSHOOT) INIT_BALL_HEIGHT = -2;
                else INIT_BALL_HEIGHT = height * 0.2f;
                INIT_BALL_GAB = 5; 
            }
            else if (pitcher.pitchingType == CPlayer._SIDEARM)
            {
                //사이드
                if (ballType == PitchingArsenal.UPSHOOT) INIT_BALL_HEIGHT = 0;
                else INIT_BALL_HEIGHT = height * 0.5f;
                INIT_BALL_GAB = 5; 
            }
            else
            {
                //오버핸드
                INIT_BALL_HEIGHT = height;
                INIT_BALL_GAB = 0; 
            }
        }//INIT_BALL_GAB
        ...
        private void setMoveMent(Pitcher pitcher)
        {
            ...
            //슬라이더
            pMovement1 = 1.5f + (0.04f * value); //1.5 ~ 5.5
            
            //커브
            pMovement2 = 1.0f + (0.045f * value); //1 ~ 5.5
            
            //offspeed
            pMovement3 = 2.0f + (0.035f * value); //2 ~ 5.5  포크 팜볼  * 1.4
            //일부 오프스피드 볼 조정
            if (ballType == PitchingArsenal.PALM || ballType == PitchingArsenal.KNUCKLE || ballType == PitchingArsenal.FORK)
            {
                pMovement3 *= 1.4f;
            }
            
            //패스트 브레이크
            pMovement4 = 3.0f + (0.07f * value); ; //3 ~ 10
            

            //pMovement1 = pMovement2 = pMovement3 = pMovement4 = testMovement;
        ...
        public void setVector()
        {
            ...
            ////Debug.Log("================>>PitchSystem ballSpeed = " + ballSpeed);
            ...
            perfectTime = dTime / ballTypeSpeedRate;
        ...
        private IEnumerator setTailing()
        {
            ...
            while (true)
            {
                Util.Load("MainGame/prefabs/BattingViewPrefab/ballTracePrefab", transform, ball.transform.localPosition).transform.localScale = (ballObjSpr.transform.localScale * 2);
        ...
        void setFastbreakDetail()
        {
            ...
            else if (ballType == PitchingArsenal.RISING)
            {
                ...
                movementAX = 0;// -14 * pHandSign * pMovement4;
        ...
        void move()
        {
            ...
            {
                float deltaTime = (Time.deltaTime * ballTypeSpeedRate);

                x += dx * deltaTime;// Time.deltaTime;
                y += dy * deltaTime;//Time.deltaTime;
                z += dz * deltaTime;//Time.deltaTime;


                dy += gravity_acc * deltaTime;//(Time.deltaTime); //dy += GRAVITY_ACC * (Time.deltaTime);// *Time.deltaTime);
        ...
        private void moveDetail(float deltaTime)
        {
            if (moveType == BallMoveType.Straight)
            {
                if (bRising == true)
                {
                    if (z < breakPointZ)
                    {
                        ...
                        if (bZoneCheck == false)
                        {
                            zoneMoveX += (zoneDX * deltaTime);
                            zoneMoveY += (zoneDY * deltaTime);
                        }
                    }
                }
            }
        ...
        {
            return (int)(speedGun / speedRate);///speedRate);
        }
        ...
        {
            IngameUI.GetPitchUI().SetActive(false);//cursorBall.renderer.enabled = false;
        ...
        public void setIndicator(Pitcher pitcher)
        {
         /*   if (pitcher.pitchingType == 2)
            ...
            cursorBall.transform.position = finalVector;// new Vector3(finalVector.x, finalVector.y, cursorBall.transform.position.z);
            PitchUI.SetActive(true);//cursorBall.renderer.enabled = true;
            PitchUI.InitRotate(moveType, ballType, pitcher.nSign);
            indicator.makeLine(pos, indicatorLineNum);
            cursorInitVector = finalVector;
            */


            IngameUI.GetPitchUI().SetActive(true);//cursorBall.renderer.enabled = true;
        ...
        public Vector3 getBallHitPos(float hitZ)
        {
            return new Vector3(x, y, hitZ);
        }
