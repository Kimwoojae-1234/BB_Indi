Shader "Game/UI/Additive"
{
    Properties
    {
        [PerRendererData] _MainTex ("Texture", 2D) = "white" {}
        _Color ("Color", Color) = (1,1,1,1)
        _TintColor ("Legacy Particle Tint", Color) = (0.5,0.5,0.5,0.5)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct v2f { float4 vertex : SV_POSITION; fixed4 color : COLOR; float2 uv : TEXCOORD0; half4 mask : TEXCOORD1; UNITY_VERTEX_OUTPUT_STEREO };
            sampler2D _MainTex;
            fixed4 _Color, _TintColor, _TextureSampleAdd;
            float4 _ClipRect;
            ...
            v2f vert(appdata input)
            {
                ...
                output.mask = half4(input.vertex.xy * 2 - clipRect.xy - clipRect.zw,
                    .25 / (.25 * half2(_UIMaskSoftnessX, _UIMaskSoftnessY) + abs(pixelSize.xy)));
                return output;
            }
            fixed4 frag(v2f input) : SV_Target
            {
                fixed4 color = (tex2D(_MainTex, input.uv) + _TextureSampleAdd) * input.color * _TintColor * 2;
