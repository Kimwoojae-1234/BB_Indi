...
현재 로비·선수·리그 화면은 UGUI/TMP 기반이고, 실제 경기 화면에 NGUI 의존성이 집중되어 있다. 따라서 실제 사용하는 경기 UI를 작은 기능 단위로 전환하고 매 단계에서 기존 동작과 비교한다. 번역 데이터 정리, 언어 선택 기능, 다국어 폰트 체계 구축은 선행 조건으로 두지 않는다. 이전에 제안한 “다국어 기반 우선” 순서는 이 우선순위로 대체한다. 경기 조작의 tk2d는 NGUI 의존 부분을 제거할 때 필요한 연결을 보존하며, 전체 재작성은 별도 범위로 남긴다.
