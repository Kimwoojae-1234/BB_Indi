"""Step 5: extend stable-ID layout and batches to the remaining 14 UGUI prefabs.

Only applies to the committed step-4 prefabs. Controllers/animation paths stay put;
...
def apply(path):
    ...
    assert before == original.replace('\r\n', '\n'), 'Input differs from step 4: ' + str(path)
    ...
    root_tr = '0'  # Dynamic prefab roots also use pixel-based RectTransforms.
    asset_root = next(tr for tr in nodes if parents[tr] == '0')
    ...
    binding_id = '890059000000000001'
    assert binding_id not in blocks
    binding_guid = script_guid('GameUIDynamicUI')
    root_go = nodes[asset_root]
    blocks[root_go] = blocks[root_go].replace('  m_Layer:', '  - component: {fileID: ' + binding_id + '}\n  m_Layer:', 1)
    blocks[binding_id] = header(114, binding_id, 'MonoBehaviour', root_go) + '  m_Enabled: 1\n  m_EditorHideFlags: 0\n  m_Script: {fileID: 11500000, guid: ' + binding_guid + ', type: 3}\n  m_Name:\n  m_EditorClassIdentifier:\n'
    ...
    return dict(source=path.relative_to(ROOT).as_posix(), beforeSha256=hashlib.sha256(before.encode()).hexdigest(),
                ...
                layoutComponents=list(widgets), removedCanvases=removed, batches=batches, dynamicBinding=binding_id, rootTransform=asset_root,
...
if __name__ == '__main__':
    ...
    results = [apply(ROOT / path) for path in scope['assets']]
    (AUDIT / 'Step5/layout-mapping.json').write_text(json.dumps(dict(baseline=BASE, assets=results), indent=2) + '\n', encoding='utf-8')
    for item in results:
